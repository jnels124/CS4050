/*  user enters p, q, sees n, phi, enters e, 
    check gcd(phi,e)=1, see d, enter repeated
    a and see c followed by a check
*/

import java.util.Scanner;
import java.math.BigInteger;

public class RSA
{
    private static BigInteger one=new BigInteger("1");
    private static BigInteger two=new BigInteger("2");
    private static BigInteger zero=new BigInteger("0");

    public static void main(String[] args)
    {
        BigInteger one=new BigInteger("1");
        BigInteger two=new BigInteger("1");

        Scanner keys = new Scanner( System.in );
        BigInteger p, q, n, phi, e, d, a, c, aCheck;

        System.out.print("enter p, q: ");
        p = keys.nextBigInteger();
        q = keys.nextBigInteger();

        n = p.multiply(q);
        phi = p.subtract(one).multiply( q.subtract(one) );
        System.out.println("n= " + n + " phi = " + phi );

        System.out.print("enter e: ");
        e = keys.nextBigInteger();

        System.out.println("gcd(phi,e) = " + phi.gcd( e ) );

        BigInteger[] info = gcd( phi, e );
        System.out.println("gcd(phi,e)=" + info[0] +
                " phi multiplier= " + info[1] + " e multiplier= " + info[2] );
        d = info[2];
        System.out.println("d= " + d );
/*
    d = e.modInverse( n );
    System.out.println("d= " + d );
*/

        System.out.println("d*e in Zn = " + d.multiply(e).mod( n ) );

        do{
            System.out.print("enter message a: ");
            a = keys.nextBigInteger();

            c = a.modPow( e, n );
            System.out.print("encrypted message is: " + c );

            aCheck = c.modPow( d, n );
            System.out.println(" unencrypted message is: " + aCheck );

        }while( true );

    }

    public static BigInteger[] gcd( BigInteger a, BigInteger b )
    {
        BigInteger[] x = new BigInteger[3];
        if( a.equals( zero ) )
        {
            x[0] = b;  x[1] = zero; x[2] = one;
        }
        else if( b.equals(zero) )
        {
            x[0] = a;  x[1] = one;  x[2] = zero;
        }
        else
        {
            BigInteger q = a.divide(b), r = a.mod(b);
            BigInteger[] xprime = gcd( b, r );

            x[0] = xprime[0];
            x[1] = xprime[2];  x[2] = xprime[1].subtract( q.multiply(xprime[2]));
        }

        System.out.println("gcd on " + a + " " + b + " gives " + x[0] + " " +
                x[1] + " " + x[2] );

        return x;
    }

}